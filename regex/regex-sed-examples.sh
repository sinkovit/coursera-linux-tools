...
# backreferences - replacing repeated words with single instance
cat repeat.txt
sed -E 's/\<([a-z]+)[[:space:]]+\1\>/**\1**/g' repeat.txt
sed -E 's/\<([a-z]+)[[:space:]]+\1[[:space:]]+\1\>/***\1***/g' repeat.txt
sed -E 's/\<([a-z]+)[[:space:]]+\1[[:space:]]+\1[[:space:]]+\1\>/****\1****/g' repeat.txt

# backreferences - swapping first two fields where
# (1) assuming delimiters are single spaces
# (2) preserving spacing between first two words
# (3) preserving space and punctuation between first two words
cat ordered1.txt
sed -E 's/([[:alnum:]]+)[[:space:]]+([[:alnum:]]+)/\2 \1/' ordered1.txt
awk '{t=$1; $1=$2; $2=t; for(i=1;i<=NF;i++) printf "%s%s",$i,(i<NF?OFS:ORS)}' ordered1.txt

cat ordered2.txt
sed -E 's/([[:alnum:]]+)([[:space:]]+)([[:alnum:]]+)/\3\2\1/' ordered2.txt
sed -E 's/([[:alnum:]]+)[[:space:]]+([[:alnum:]]+)/\2 \1/' ordered2.txt
